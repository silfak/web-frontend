            ...
            <>
              <div>
                ...
                <select
                  ...
                  onChange={(e) => onChange("gedung", e.target.value)}
                  className="w-full border border-gray-300 p-2 rounded-lg focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-700"
                >
                  <option value="">Pilih Gedung</option>
                  {gedungList.map((g, i) => (
                    <option key={i} value={g.nama}>{g.nama}</option>
                  ))}
                </select>
              </div>
              <div>
            ...
            <>
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-1 flex items-center gap-1">
                  Isi field ini jika hanya ingin mengubah password {editType === "user" ? "OB" : "Admin"}
                </p>
              </div>
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    type="button"
                    onClick={() => onChange("status", !editItem?.status)}
                    className={`w-10 h-5 flex items-center rounded-full p-1 transition-colors ${
                      editItem?.status ? "bg-green-600" : "bg-gray-300"
                    }`}
                  >
                    <div className={`w-4 h-4 bg-white rounded-full transition-transform ${
                      editItem?.status ? "translate-x-5" : ""
                    }`} />
                  </button>
                  <span className={`text-xs font-medium px-2 py-1 rounded-full ${
                    editItem?.status ? "bg-green-100 text-green-700" : "bg-red-100 text-red-600"
                  }`}>
