  ...
  useEffect(() => {
    if (selectedBuilding) {
      const filtered = allRooms.filter(
        room => String(room.buildingId) === String(selectedBuilding)
      );
        ...
        <div className="p-8 space-y-8 max-h-[80vh] overflow-y-auto text-sm">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <select
                  ...
                  disabled={!selectedBuilding || filteredRooms.length === 0}
          ...
          <div className="space-y-4">
            ...
            {!selectedFile ? (
              ...
              <div className="bg-[#E8F5EE] border border-green-100 rounded-2xl p-4 flex items-center gap-4 animate-in slide-in-from-top-2 duration-300">
          ...
          <button
            ...
            disabled={loading}
            className="w-full bg-[#107C41] text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-[#0d6334] transition-all shadow-lg active:scale-[0.98] disabled:opacity-60"
          >
            <Send size={18} /> {loading ? "Mengirim..." : "Kirim Laporan"}
          </button>
          <p className="text-center text-[10px] text-gray-400 italic">
            Laporan akan diverifikasi oleh admin fasilitas dalam waktu 1x24 jam.
          </p>
        </div>
      </div>
    </div>
