...
import {
  Inbox, Clock, CheckCircle, Download,
  FileDown, BarChart3, MapPin, CheckCircle2, X
} from "lucide-react";
...
export default function DashboardView({ laporanData = [] }) {
  ...
  const topRuangan = Object.entries(ruanganCount)
    .map(([key, total]) => {
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="bg-white p-4 rounded-xl shadow-lg">
          <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-green-100 mb-3">
            <Inbox className="text-green-600" />
          </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-lg">
          <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-yellow-100 mb-3">
              ...
              {topRuangan.map((item, index) => (
                <div key={index} className="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                  <div className="flex items-center gap-3">
                    ...
                    {/* NOMOR */}
                    <div className={`w-7 h-7 flex items-center justify-center rounded-lg text-xs font-bold transition-all cursor-default
                      ${index === 0 ? " text-black hover:bg-pink-200 hover:text-pink-600" :
                        index === 1 ? "text-black hover:bg-pink-200 hover:text-pink-600" :
                        "text-black hover:bg-pink-100 hover:text-pink-500"}`}>
                      {index + 1}
                    </div>
        ...
        <div className="bg-white p-4 rounded-xl shadow-lg flex flex-col sm:flex-row justify-between gap-4 sm:items-center">
          ...
          <div className="flex items-center gap-3">
            <div className="w-10 h-10 flex items-center justify-center rounded-lg bg-green-100">
              <FileDown className="text-green-700" size={18} />
            </div>
            <div>
          ...
          <div className="flex flex-col sm:flex-row gap-2 w-full sm:w-auto">
           ...
           <option key={bulan}>{bulan}</option>
            ...
            <button
              ...
              className="bg-green-700 text-white px-4 py-2 rounded-lg flex items-center justify-center gap-2 text-sm hover:bg-green-800 transition-all"
            >
              <Download size={16} />
              Export Data
            </button>
