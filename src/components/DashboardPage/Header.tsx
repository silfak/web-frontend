...
export default function Header({ title, onProfileClick, onViewDetail, reports, onOpenSidebar }: any) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("silfak_last_notif_mhs");
  ...
  const handleToggleNotif = () => {
    ...
    if (newState) {
      const now = Date.now();
      setLastReadTime(now);
      localStorage.setItem("silfak_last_notif_mhs", now.toString());
    }
  };
  ...
  const getNotifConfig = (report: any) => {
    if (report.status === "Reported") {
      return {
        text: `Laporan baru: ${report.masalah} di ${report.lokasi} berhasil dicatat.`,
        icon: <Flag size={20} />,
        color: "text-blue-600",
        bg: "bg-blue-100"
      };
      ...
      return {
        text: `Laporan ${report.masalah} di ${report.lokasi} sekarang sedang ditangani (In Progress).`,
      ...
      return {
        text: `Laporan ${report.masalah} di ${report.lokasi} telah selesai ditangani (Resolved).`,
    ...
    <div className="flex justify-between items-center mb-6 md:mb-8 pb-4 border-b border-gray-200 relative">
      ...
      <div className="flex gap-4 md:gap-6 items-center text-gray-400">
        <div className="relative flex items-center">
          ...
          {showNotif && (
            <div className="absolute top-full right-0 mt-4 w-80 bg-white shadow-2xl rounded-3xl border border-gray-100 z-50 animate-in slide-in-from-top-2 duration-300 max-h-[500px] flex flex-col overflow-hidden">
              <div className="absolute -top-2 right-2 w-4 h-4 bg-white rotate-45 border-t border-l border-gray-50"></div>

              <div className="p-6 pb-2 relative border-b border-gray-100 shrink-0">
                ...
                {recentReports.length === 0 ? (
                  <div className="py-10 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-[11px] text-gray-400 px-6 leading-relaxed">
                      Notifikasi akan muncul saat ada laporan baru atau status berubah.
                    </p>
                  </div>
                ) : (
