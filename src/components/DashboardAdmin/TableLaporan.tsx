...
interface TableLaporanProps {
  ...
  onLihatDetail?: (item: LaporanAdmin) => void;
}

export default function TableLaporan({ data = [], page = 1, setPage, onLihatDetail }: TableLaporanProps) {
  ...
  const getStatusStyle = (status: LaporanAdminStatus): StatusStyle => {
    switch (status) {
      case "reported": return { text: "Reported", className: "bg-blue-100 text-blue-600", icon: <Flag size={14} /> };
      case "inprogress": return { text: "In Progress", className: "bg-yellow-100 text-yellow-600", icon: <ClockFading size={14} /> };
      case "resolved": return { text: "Resolved", className: "bg-green-100 text-green-600", icon: <CheckCircle size={14} /> };
      default: return { text: status || "-", className: "bg-gray-100 text-gray-600", icon: null };
    }
  };
    ...
    <div className="bg-white rounded-xl shadow-lg p-4">
            ...
            return (
              <div key={i} className="border rounded-lg p-3 shadow-sm">
                <div className="flex justify-between items-center">
                  ...
                  <span className={`flex items-center gap-1 px-2 py-1 text-xs rounded-full ${status.className}`}>{status.icon}{status.text}</span>
                </div>
                ...
                <button onClick={() => onLihatDetail?.(item)} className="text-green-700 text-sm font-semibold mt-2 flex items-center gap-1">
      ...
      <div className="hidden md:block overflow-x-auto">
        <table className="min-w-[700px] w-full text-sm">
          ...
          <tbody>
            {currentData.length === 0 ? (
              <tr><td colSpan={7} className="text-center p-6 text-gray-400">Tidak ada data</td></tr>
            ) : (
              currentData.map((item, i) => {
                const status = getStatusStyle(item?.status);
                return (
                  <tr key={i} className="border-b hover:bg-gray-50">
                    ...
                    <td className="p-3">
                      <span className={`flex items-center gap-1 px-3 py-1 text-xs rounded-full w-fit ${status.className}`}>{status.icon}{status.text}</span>
                    </td>
                    <td className="p-3">
                      <button onClick={() => onLihatDetail?.(item)} className="flex items-center gap-1 text-green-700 font-semibold text-sm hover:text-green-900 transition-colors">
                        Lihat Detail <ArrowRight size={14} />
                      </button>
                    </td>
                  </tr>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center mt-4 text-sm gap-2">
        ...
        <div className="flex gap-2 flex-wrap">
          ...
          {[...Array(totalPages)].map((_, i) => (
            <button key={i} onClick={() => setPage(i + 1)} className={`px-3 py-1 rounded ${page === i + 1 ? "bg-green-700 text-white" : "border"}`}>{i + 1}</button>
          ))}
