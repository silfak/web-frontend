    ...
    <div className="bg-white p-4 rounded-xl shadow-lg">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <div>
          ...
          {/*  Preview badge (opsional biar mirip figma) */}
          {filters.status && (
            <div className="mt-2">
              {statusOptions
                ...
                .map((s) => (
                  <span
                    key={s.value}
                    className={`flex items-center gap-1 px-3 py-1 text-xs rounded-full w-fit bg-gray-100 ${s.className}`}
                  >
                    {s.icon}
                    {s.label}
                  </span>
