...
export default function ToggleUserModal({
  show,
  user,
  onClose,
  onConfirm,
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50">
      <div className="bg-white rounded-2xl p-6 w-[360px] text-center shadow-xl">
        ...
        <div className={`w-14 h-14 flex items-center justify-center rounded-xl mx-auto mb-4 ${
          user?.status ? "bg-red-100" : "bg-green-100"
        }`}>
        ...
        <h3 className="font-semibold text-base mb-2">
          {user?.status
            ? `Nonaktifkan Akun ${getRoleLabel()} Ini?`
            : `Aktifkan Akun ${getRoleLabel()} Ini?`}
        </h3>
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className={`flex-1 text-white py-2 rounded-lg ${
              user?.status ? "bg-red-500 hover:bg-red-600" : "bg-green-600 hover:bg-green-700"
            }`}
          >
