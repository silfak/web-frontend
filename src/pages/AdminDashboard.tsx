...
export default function AdminDashboard() {
  ...
  const dummyData: LaporanAdmin[] = Array.from({ length: 20 }, (_, i) => {
    ...
    return {
      ...
      tanggal: new Date(2026, 3, Math.floor(Math.random() * 28) + 1).toLocaleDateString("id-ID"),
      jam: `${14 + Math.floor(Math.random() * 5)}:${Math.floor(Math.random() * 60).toString().padStart(2, "0")}`,
      status: statuses[Math.floor(Math.random() * statuses.length)],
    };
  });

  const laporanData = dummyData;

  const renderContent = () => {
    switch (activeMenu) {
      case "dashboard": return <DashboardView laporanData={laporanData} />;
      case "laporan": return <LaporanView />;
      case "manajemen": return <ManajemenView />;
      case "profile": return <ProfileAdminView />;
      default: return <DashboardView laporanData={laporanData} />;
    }
  };
